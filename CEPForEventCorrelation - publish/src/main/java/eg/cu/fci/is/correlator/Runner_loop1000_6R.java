...
public class Runner_loop1000_6R {

    public static void main(String[] args) throws InterruptedException, IOException {


    	String labeledEvents = "C:\\IH\\eclipse-workspace\\CEPForEventCorrelation\\loop1000_6R_labeled.csv";
        ...
        String epl_TE_f =" @Priority(5) @Name('TE_f') insert into TempEvent (caseID, activity, timestamp) select distinct pred.caseID, 'f' as activity, succ.timestamp as timestamp from pattern [(every pred=FilterLabeledEvent(activity = 'b') -> every succ=UnlabeledEvent(activity='f')) where timer:within(121 sec)]#time(121 sec)  where (succ.timestamp - pred.timestamp) >= 120000 and (succ.timestamp - pred.timestamp) <=120000;";

        String epl_TE_g =" @Priority(5) @Name('TE_g') insert into TempEvent (caseID, activity, timestamp) select distinct pred.caseID, 'g' as activity, succ.timestamp as timestamp from pattern [(every pred=FilterLabeledEvent(activity = 'c') -> every succ=UnlabeledEvent(activity='g')) where timer:within(61 sec)]#time(61 sec)  where (succ.timestamp - pred.timestamp) >= 60000 and (succ.timestamp - pred.timestamp) <=60000;";
