...
public class Runner_BPI2013Closed {
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
    	...
    	String labeledEvents = "C:\\IH\\Docs\\eclipse-ws2019\\CEPForEventCorrelation\\BPI2013Closed_labeled.csv";
        ...
        String epl_start_case_accepted =" @Name('start_case_accepted')  insert into FilterLabeledEvent (caseID, activity, timestamp, probability)  select (select coalesce(max(caseID)+1,1) from Cases) as caseID, UE.activity, UE.timestamp, 1.0  from UnlabeledEvent as UE where UE.activity = 'accepted'; ";
        ...
        String epl_Filter_queued =" @Priority(50) @Name('Filter_queued') insert into FilterLabeledEvent (caseID, activity , timestamp, probability) select distinct caseID as caseID, last(activity) as activity, max(timestamp) as timestamp , probability from LabeledEvent.win:time(1 sec) as T where activity = 'queued'  and probability >= 0.0 group by caseID order by probability desc limit 5 ;";

        String epl_Filter_unmatched =" @Priority(50) @Name('Filter_unmatched') insert into FilterLabeledEvent (caseID, activity , timestamp, probability) select distinct caseID as caseID, last(activity) as activity, max(timestamp) as timestamp , probability from LabeledEvent.win:time(1 sec) as T where activity = 'unmatched'  and probability >= 0.0 group by caseID order by probability desc limit 5 ;";

        String epl_Filter_completed =" @Priority(50) @Name('Filter_completed') insert into FilterLabeledEvent (caseID, activity , timestamp, probability) select distinct caseID as caseID, last(activity) as activity, max(timestamp) as timestamp , probability from LabeledEvent.win:time(1 sec) as T where activity = 'completed'  and probability >= 0.0 group by caseID order by probability desc limit 5 ;";
