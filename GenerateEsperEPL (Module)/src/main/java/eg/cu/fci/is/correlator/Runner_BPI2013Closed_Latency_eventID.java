...
public class Runner_BPI2013Closed_Latency_eventID {
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
    	...
    	String labeledEvents = "C:\\IH\\Docs\\eclipse-ws2019\\CEPForEventCorrelation_latency\\BPI2013Closed_labeled_latency_prob0.4_eventID.csv";
...
//        
//        LabeledEvent.put("caseID", long.class);
//        LabeledEvent.put("activity", String.class);
//        LabeledEvent.put("timestamp", long.class);
//        LabeledEvent.put("probability", double.class);
//        LabeledEvent.put("sysTime", long.class);
//        
//        TempEvent.put("caseID", long.class);
//        TempEvent.put("activity", String.class);
//        TempEvent.put("timestamp", long.class);
//        TempEvent.put("sysTime", long.class);
//        
//        FilterLabeledEvent.put("caseID", long.class);
//        FilterLabeledEvent.put("activity", String.class);
//        FilterLabeledEvent.put("timestamp", long.class);
//        FilterLabeledEvent.put("probability", double.class);
//        FilterLabeledEvent.put("initTime", long.class);
//        FilterLabeledEvent.put("sysTime", long.class);
        
//        System.out.println("UnlabeledEvent map == Runner == "+UnlabeledEvent.toString());
//        // Here you register your event types
//        configuration.getCommon().addEventType("Cases", Cases);
//        configuration.getCommon().addEventType("UnlabeledEvent", UnlabeledEvent);
//        configuration.getCommon().addEventType("FilterLabeledEvent", FilterLabeledEvent);
//        configuration.getCommon().addEventType("LabeledEvent", LabeledEvent);
//        configuration.getCommon().addEventType("TempEvent", TempEvent);
        ...
        String epl_start_case_accepted =" @Name('start_case_accepted')  insert into FilterLabeledEvent (eventID, caseID, activity, timestamp, probability, initTime)  select UE.eventID, (select coalesce(max(caseID)+1,1) from Cases) as caseID, UE.activity, UE.timestamp, 1.0, UE.sysTime as initTime  from UnlabeledEvent as UE where UE.activity = 'accepted'; ";

        String epl_TE_queued =" @Priority(5) @Name('TE_queued') insert into TempEvent (eventID, caseID, activity, timestamp) select distinct succ.eventID, pred.caseID, 'queued' as activity, succ.timestamp as timestamp from pattern [(every pred=FilterLabeledEvent(activity = 'accepted')) -> every succ=UnlabeledEvent(activity='queued') ]#time(86078 sec)  where succ.timestamp - pred.timestamp >= 0 and succ.timestamp - pred.timestamp <=86078000;";

        String epl_TE_unmatched =" @Priority(5) @Name('TE_unmatched') insert into TempEvent (eventID, caseID, activity, timestamp) select distinct succ.eventID, pred.caseID, 'unmatched' as activity, succ.timestamp as timestamp from pattern [(every pred=FilterLabeledEvent(activity = 'accepted')) -> every succ=UnlabeledEvent(activity='unmatched') ]#time(75866 sec)  where succ.timestamp - pred.timestamp >= 0 and succ.timestamp - pred.timestamp <=75866000;";

        String epl_TE_completed =" @Priority(5) @Name('TE_completed') insert into TempEvent (eventID, caseID, activity, timestamp) select distinct succ.eventID, pred.caseID, 'completed' as activity, succ.timestamp as timestamp from pattern [(every pred=FilterLabeledEvent(activity='accepted' or activity='unmatched')) -> every succ=UnlabeledEvent(activity='completed') ]#time(86396 sec)  where succ.timestamp - pred.timestamp >= 0 and succ.timestamp - pred.timestamp <=86396000;";

        String epl_Correlate_queued =" @Priority(20) @Name('Correlate_queued') insert into LabeledEvent (eventID, caseID, activity , timestamp, probability) select distinct T.eventID, caseID as caseID, last(activity) as activity, max(timestamp) as timestamp , min(((select count(caseID)  from TempEvent.win:time_batch(1 sec) where caseID = T.caseID and activity = T.activity and timestamp=T.timestamp) /count(all caseID, group_by:timestamp)),1.0) as probability from TempEvent.win:time_batch(1 sec) as T  where activity = 'queued'  group by caseID order by timestamp; ";

        String epl_Correlate_unmatched =" @Priority(20) @Name('Correlate_unmatched') insert into LabeledEvent (eventID, caseID, activity , timestamp, probability) select distinct T.eventID, caseID as caseID, last(activity) as activity, max(timestamp) as timestamp , min(((select count(caseID)  from TempEvent.win:time_batch(1 sec) where caseID = T.caseID and activity = T.activity and timestamp=T.timestamp) /count(all caseID, group_by:timestamp)),1.0) as probability from TempEvent.win:time_batch(1 sec) as T  where activity = 'unmatched'  group by caseID order by timestamp; ";

        String epl_Correlate_completed =" @Priority(20) @Name('Correlate_completed') insert into LabeledEvent (eventID, caseID, activity , timestamp, probability) select distinct T.eventID, caseID as caseID, last(activity) as activity, max(timestamp) as timestamp , min(((select count(caseID)  from TempEvent.win:time_batch(1 sec) where caseID = T.caseID and activity = T.activity and timestamp=T.timestamp) /count(all caseID, group_by:timestamp)),1.0) as probability from TempEvent.win:time_batch(1 sec) as T  where activity = 'completed'  group by caseID order by timestamp; ";

        String epl_Filter_queued =" @Priority(50) @Name('Filter_queued') insert into FilterLabeledEvent (eventID, caseID, activity , timestamp, probability, initTime) select distinct T.eventID, caseID as caseID, last(activity) as activity, max(timestamp) as timestamp , probability, "+initTime+"   from LabeledEvent.win:time(1 sec) as T where activity = 'queued'  and probability >= 0.4 group by caseID order by probability desc limit 5 ;";

        String epl_Filter_unmatched =" @Priority(50) @Name('Filter_unmatched') insert into FilterLabeledEvent (eventID, caseID, activity , timestamp, probability, initTime) select distinct T.eventID, caseID as caseID, last(activity) as activity, max(timestamp) as timestamp , probability, "+initTime+" from LabeledEvent.win:time(1 sec) as T where activity = 'unmatched'  and probability >= 0.4 group by caseID order by probability desc limit 5 ;";

        String epl_Filter_completed =" @Priority(50) @Name('Filter_completed') insert into FilterLabeledEvent (eventID, caseID, activity , timestamp, probability, initTime) select distinct T.eventID, caseID as caseID, last(activity) as activity, max(timestamp) as timestamp , probability, "+initTime+"  from LabeledEvent.win:time(1 sec) as T where activity = 'completed'  and probability >= 0.4 group by caseID order by probability desc limit 5 ;";
...
*/
        ...
        String filePath = "C:\\IH\\Docs\\eclipse-ws2019\\CEPForEventCorrelation_latency\\BPI2013Closed_unLabeled_eventID.csv";
//        new UnlabeledEventSource_MapEvent1(filePath, 6661, eventService, runtime, "UnlabeledEvent").run();//take 35 records from UnlabeledEvent
        new UnlabeledEventSource_eventID(filePath, 6661, eventService, "UnlabeledEvent").run();//take 35 records from UnlabeledEvent
